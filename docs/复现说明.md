...
输出为`1_inferenced_npy`

存储在 .npy 中的数值严格遵循证据理论（Evidential Deep Learning）的定义：
...
地图可视化
```bash
roslaunch evsemmap map_reader.launch map_path1:=/home/xzy/deployTest/rellis_map.map
```